    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-400 text-sm">
          Comece adicionando a primeira atividade do seu bebê!
        </p>
      </div>
    );
  }
        ...
        return (
          <div key={activity.id} className="flex items-start space-x-4 group">
            ...
            <div className="flex flex-col items-center">
              <div
                className={`p-2 rounded-xl bg-gradient-to-r ${config.color} shadow-lg group-hover:scale-110 transition-transform duration-200`}
              >
            ...
            {/* Activity content */}
            <div
              className={`flex-1 p-4 rounded-xl ${config.bgColor} border transition-all duration-200 hover:shadow-lg`}
            >
              ...
              <div className="space-y-1 text-sm text-gray-600 flex justify-between">
                ...
                {activity.type === "sleep" && activity.duration && (
                  <p>
                    Duração: {Math.floor(activity.duration / 60)}h{" "}
                ...
                {activity.type === "medicine" && (
                  <>
                    {activity.details.name && (
                      <p>Medicamento: {activity.details.name}</p>
                    )}
                    {activity.details.dosage && (
                      <p>Dosagem: {activity.details.dosage}</p>
                    )}
                  </>
                )}
                ...
                <span className="text-sm text-gray-500">
                {format(new Date(activity.timestamp), "dd, EEEE", {
                  locale: ptBR,
                })}
              </span>
              </div>
